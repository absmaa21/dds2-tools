  ...
  return (
    <div className={"shop-item-container"}>
      <Row>
        ...
        {item.amount &&
            <Typography fontSize={11} fontWeight={700} color={'#fffa'}>{`${item.amount} ${item.unit}`}</Typography>}
      </Row>

      <Row>
        <Typography fontSize={12}
                    color={'#fffa'}>B {getDiscountedPrice(item.price, shop.discounts[level]).toLocaleString()}</Typography>
        <Typography fontSize={12}
                    color={'#fffa'}>x{item.quantity[level]?.toLocaleString() ?? ' unknown quantity'}</Typography>
      </Row>
